...
-(void)setAnnotionsWithList:(NSArray *)list
{
    for (NSDictionary *dic in list) {
        ...
        BasicMapAnnotation *annotation=[[[BasicMapAnnotation alloc] initWithLatitude:latitude andLongitude:longitude]  autorelease];
			 ...
			 annotation.tel = [dic objectForKey:@"tel"];
...
- (void)mapView:(MKMapView *)mapView didSelectAnnotationView:(MKAnnotationView *)view {
	if ([view.annotation isKindOfClass:[BasicMapAnnotation class]]) {
		...
		BasicMapAnnotation *baseAnot = (BasicMapAnnotation *)view.annotation;
		
		_calloutAnnotation.name = baseAnot.title;
		_calloutAnnotation.address = baseAnot.address;
		_calloutAnnotation.tel = baseAnot.tel;
...
- (MKAnnotationView *)mapView:(MKMapView *)mapView viewForAnnotation:(id <MKAnnotation>)annotation {
	if ([annotation isKindOfClass:[CalloutMapAnnotation class]]) {
		...
		annotationView.frame = CGRectMake(annotationView.frame.origin.x, annotationView.frame.origin.y - 10, cell.frame.size.width, cell.frame.size.height + 10);
		
		[annotationView addSubview:cell];
			CalloutMapAnnotation *outMap = (CalloutMapAnnotation *)annotation;
		
		 UILabel *lab_Name = (UILabel *)[cell viewWithTag:100];
		 lab_Name.text = outMap.name;
		 UILabel *lab_Address = (UILabel *)[cell viewWithTag:101];
		 lab_Address.text = outMap.address;
		 UILabel *lab_Tel = (UILabel *)[cell viewWithTag:102];
		 lab_Tel.text = outMap.tel;
